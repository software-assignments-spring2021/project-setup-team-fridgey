...
router.post("/SaveRecipe", async (req, res) => {
  ...
  try {
    var currentSavedList = await SavedRecipe.find({
      _id: "607e927189dc61405c4f6e5a",
    });
    currentSavedList[0].ids = pushIfNotAlreadyExists(
      req.body.id,
      currentSavedList[0].ids
    );
...
router.post("/RemoveRecipe", async (req, res) => {
  try {
    ...
    currentSavedList[0].ids = currentSavedList[0].ids.filter(
      (id) => id != req.body.id
    );
